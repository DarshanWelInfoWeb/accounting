...
Future<Uint8List> makeSupplierInvoicePdf(String customerName,String address,String gstNo,SupplierInvoiceData1 tempInvoiceData, List<SupplierInvoiceItemData1> tempStockList, double totalAmount, double total, double cgst, double sgst,double cgstValue,double sgstValue,double gstValue) async {
  final pdf = Document();
        ...
        return [
            ...
            Table(
              ...
              children: [
                TableRow(
                  children: [
                    paddedText('INVOICE NO : ${tempInvoiceData.strInvoiceNo}'),
                    paddedText('Date : ${AppConstants.date_chang(tempInvoiceData.strInvoiceDate.toString())}')
                  ],
                ),
                TableRow(
                  children: [
                    ...
                    p.TableRow(
                        children: [
                          ...
                          p.Container(
                              alignment: p.Alignment.center,
                              padding: const p.EdgeInsets.all(05),
                              child: p.Text("${tempStockList[i].intqty?.round().toString()}",style: const p.TextStyle(fontSize: 10))
                          ),
                          p.Container(
                              alignment: p.Alignment.center,
                              padding: const p.EdgeInsets.all(05),
                              child: p.Text("${tempStockList[i].decRate?.round().toString()}",style: const p.TextStyle(fontSize: 10))
                          ),
                          p.Container(
                              alignment: p.Alignment.center,
                              padding: const p.EdgeInsets.all(05),
                              child: p.Text("${tempStockList[i].totalAmount?.round().toString()}",style: const p.TextStyle(fontSize: 10))
                          ),
                        ]
                    ),
                  p.TableRow(
                  ...
                  children: [
                    TableRow(
                      children: [
                        ...
                        paddedText(totalAmount.toStringAsFixed(2)),
                      ],
                    ),
                    TableRow(
            ...
            Table(
              ...
              children: [
                TableRow(
                  children: [
                    paddedText('Rupees (In Words) : ${NumberToWordsEnglish.convert(total.round()).toUpperCase()} -/ Only'),
                  ],
                ),
              ],
            ),
