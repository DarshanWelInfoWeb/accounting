...
class EditInvoice extends StatefulWidget {
  String id,customerId,customerName,invoiceDate,dueDate,subTotal,discount,grandTotal,totalGrand,type;
  List<MainInvoiceStockBody> modelList;
  EditInvoice(this.id,this.customerId,this.customerName,this.invoiceDate,this.dueDate,this.subTotal,this.discount,this.grandTotal,this.totalGrand,this.modelList,this.type,{Key? key}) : super(key: key);
...
class _EditInvoiceState extends State<EditInvoice> {
  ...
  void initState() {
    ...
    widget.invoiceDate==""?
    dateController.text = formattedDate:
    dateController.text = widget.invoiceDate;
    widget.dueDate==""?
    dueDateController.text = date_shcedule:
    ...
    setState(() {
      ...
      invoiceStockDb.getInvoiceStockList().then((value) {
        setState(() {
          ...
          // for(int i=0;i<modelList.length;i++){
          //   print("widget:::${widget.grandTotal}:::${widget.totalGrand}");
          //   grandTotals += double.parse(modelList[i].decAmount.toString());
          //   subTotalController.text = grandTotals.round().toString();
          //   grandTotalController.text = grandTotals.round().toString();
          //   print("widget:::$grandTotal");
          // }
          print("object::::::4::${modelList.length}:::${widget.discount}");
        });
      });
    });
    print("object:::::s:4::::${widget.discount}:${widget.subTotal}::${widget.grandTotal}:::${widget.totalGrand}");
      ...
      Provider.of<InvoiceProvider>(context, listen: false).getMainInvoiceEdit(context,widget.id).then((value) {
        setState(() {
          ...
          for(int i=0;i<Provider.of<InvoiceProvider>(context, listen: false).editMainInvoiceList.length;i++){
            Provider.of<RouteProvider>(context, listen: false).getDriverHelper(context,PreferenceUtils.getString("${AppConstants.companyId}"),"3").then((value) {
              is_loading = false;
                for(int k=0;k<Provider.of<RouteProvider>(context, listen: false).driverList.length;k++){
                  if(widget.type == "edit"){
                    dropdownValue = Provider.of<InvoiceProvider>(context, listen: false).editMainInvoiceList[i].intcustomerid;
                    if(Provider.of<InvoiceProvider>(context, listen: false).editMainInvoiceList[i].intcustomerid == Provider.of<RouteProvider>(context, listen: false).driverList[k].intId){
                    ...
                    if(int.parse(widget.customerId) == Provider.of<RouteProvider>(context, listen: false).driverList[k].intId){
                      customerName = Provider.of<RouteProvider>(context, listen: false).driverList[k].srtFullName;
                    }
            ...
            invoiceNumberController.text = Provider.of<InvoiceProvider>(context, listen: false).editMainInvoiceList[i].strInvoiceNo ==null?"":Provider.of<InvoiceProvider>(context, listen: false).editMainInvoiceList[i].strInvoiceNo.toString();
            dateController.text = widget.invoiceDate==""?AppConstants.date_chang(Provider.of<InvoiceProvider>(context, listen: false).editMainInvoiceList[i].dtInvoicedate.toString()):widget.invoiceDate;
            dueDateController.text = widget.dueDate==""?AppConstants.date_chang(Provider.of<InvoiceProvider>(context, listen: false).editMainInvoiceList[i].dtDueDate.toString()):widget.dueDate;
            discountController.text = widget.discount == "0"?Provider.of<InvoiceProvider>(context, listen: false).editMainInvoiceList[i].decDiscount!.round().toString():widget.discount;
            discount = widget.discount == "0"?double.parse(Provider.of<InvoiceProvider>(context, listen: false).editMainInvoiceList[i].decDiscount!.round().toString()):double.parse(widget.discount);
          }
        });
      });
      Provider.of<InvoiceProvider>(context, listen: false).getMainInvoiceStockList(context,widget.id).then((value) {
        setState(() {
          for(int i=0;i<Provider.of<InvoiceProvider>(context, listen: false).mainInvoiceStockList.length;i++){
            subTotals += Provider.of<InvoiceProvider>(context, listen: false).mainInvoiceStockList[i].decAmount!.round();
            grandTotalScore = subTotals + grandTotals;
            grandTotal = subTotals - discount;
            subTotalController.text = grandTotalScore.round().toString();
            grandTotalController.text = grandTotalScore.round().toString();
          }
          print("stock::1:$subTotals::::$grandTotalScore::$grandTotal");
        });
      });
  ...
  _addInvoice() async {
    ...
    // }
    else if (modelList.isEmpty && Provider.of<InvoiceProvider>(context, listen: false).mainInvoiceStockList.isEmpty) {
      ...
      setState(() {
        ...
        for(int i=0;i<items.length;i++){
          ...
          Provider.of<InvoiceProvider>(context, listen: false).getDeleteMainInvoiceStock(context,items[i].intid.toString());
        }
      });
      ...
      if (response.statusCode == 200) {
        ...
        _route();
        // AppConstants.getToast('Invoice Updated Successfully');
  ...
  List<MainInvoiceStockData> tempList = [];
  int? idToRemove;
  MainInvoiceStockData removedItem = MainInvoiceStockData();
  List<MainInvoiceStockData> removedList = [];
  List<MainInvoiceStockData> items = [];
  ...
  List<DataRow> _buildRows() {
                        ...
                        TextButton(
                          onPressed: () {
                            setState(() {
                              ...
                              invoiceStockDb.deleteInvoiceStock(item).then((value) {
                                // Navigator.push(context, MaterialPageRoute(builder: (context) => EditInvoice(widget.id,widget.customerId,widget.invoiceDate,widget.dueDate,subTotal.toString(),widget.discount,grandTotal.toString(),"",modelList,"table"),));
                              });
    ...
    // Add rows from data2
    rows.addAll(Provider.of<InvoiceProvider>(context).mainInvoiceStockList.map((item) => DataRow(
      cells: [
        DataCell(GestureDetector(
          onTap: (){
            showDialog<bool>(
                      ...
                      TextButton(
                        onPressed: () {
                          setState(() {
                            ...
                            print(" :: $items");
                            items.add(item);
                            Provider.of<InvoiceProvider>(context, listen: false).mainInvoiceStockList.removeWhere((element){
                            ...
                            // Provider.of<InvoiceProvider>(context, listen: false).getDeleteMainInvoiceStock(context,item.intid.toString()).then((value) {
                              // Navigator.push(context, MaterialPageRoute(builder: (context) => EditInvoice(widget.id,widget.customerId,widget.invoiceDate,widget.dueDate,subTotal.toString(),widget.discount,grandTotal.toString(),"",modelList,"table"),));
                            // });
                            // Navigator.pop(context);
                          });
  ...
  Widget build(BuildContext context) {
    return WillPopScope(
      onWillPop: () async{
        invoiceStockDb.deleteAllInvoiceStock();
        AppConstants.closeKeyboard();
        Navigator.push(context,MaterialPageRoute(builder: (context) => InvoiceList("","",widget.customerId,"Invoice"),));
        return true;
      },
          ...
          child: Scaffold(
            ...
            ListView(
              ...
              /*  Container(
                  ...
                  child: Consumer<RouteProvider>(builder: (context, customer, child) {
                    ...
                    return DropdownButtonHideUnderline(
                        child: CustomSearchableDropDown(
                          ...
                          onChanged: (value){
                            dropdownValue = value;
                            Provider.of<RouteProvider>(context, listen: false).getItemList(context,PreferenceUtils.getString("${AppConstants.companyId}"),'$dropdownValue','$itemDropdownValue');
                ...
                CustomButtonFuction(onTap: (){
                  if (itemDropdownValue == null) {
                    ...
                    setState(() {
                      ...
                      Navigator.push(context, MaterialPageRoute(builder: (context) => EditInvoice(widget.id,dropdownValue.toString(),customerName!,dateController.text,dueDateController.text,subTotalController.text,discountController.text,grandTotalController.text,grandTotal.toString(),modelList,"table"),));
                    });
                  } else if (amountController.text == '') {
                    AppConstants.getToast('Please Enter Amount');
                  }  else {
                    bool doesItemExit = modelList.any((element) => element.name == itemsDropdownName);
                    bool doesItemExits = invoice.mainInvoiceStockList.any((element) => element.strItemName == itemsDropdownName);
                    setState(() {
                      if(!doesItemExit && !doesItemExits){
                        ...
                        // grandTotalScore = subTotal - discount;
                        // subTotalController.text = subTotal.round().toString();
                        // grandTotalController.text = grandTotalScore.round().toString();
                        grandTotal = double.parse(amountController.text.toString());
                        subTotal = grandTotal + subTotal;
                        ...
                        invoiceStockDb.insertInvoiceStock(itemDetailsInvoiceModel!).then((value) {
                              ...
                              // Provider.of<InvoiceProvider>(context, listen: false).getMainInvoiceStockList(context,widget.id).then((value) {
                                setState(() {
                                  for(int k=0;k<modelList.length;k++){
                                    for(int i=0;i<Provider.of<InvoiceProvider>(context, listen: false).mainInvoiceStockList.length;i++){
                                      subTotals = double.parse(Provider.of<InvoiceProvider>(context, listen: false).mainInvoiceStockList[i].decAmount!.round().toString());
                                      grandTotals += double.parse(modelList[k].decAmount.toString());
                                      // grandTotalScore = subTotals + grandTotals;
                                      // grandTotal = subTotals - discount;
                                      // subTotalController.text = grandTotalScore.round().toString();
                                      // grandTotalController.text = grandTotalScore.round().toString();
                          ...
                          Navigator.push(context, MaterialPageRoute(builder: (context) => EditInvoice(widget.id,dropdownValue.toString(),customerName.toString(),dateController.text,dueDateController.text,subTotalController.text,discountController.text,grandTotalController.text,grandTotal.toString(),modelList,"table"),));
                        });
                      }else{
                        itemsDropdownName = null;
                        qtyController.text = "";
                                                ...
                                                TextButton(
                                                  onPressed: () {
                                                    setState(() {
                                                      ...
                                                      subTotalController.text = grandTotal.round().toString();
                                                      grandTotalController.text = grandTotal.round().toString();
                                                      invoiceStockDb.deleteInvoiceStock(modelList[index]);
                                                      print("delete ::$grandTotal ::$subTotal");
                                                      Navigator.pop(context);
                                                    });
                                                  },
                                                  style: const ButtonStyle(
                ...
                invoice.mainInvoiceStockList.isEmpty
                    ?
                const SizedBox()
                    :
                ListView.builder(
                              ...
                              Row(
                                ...
                                children: [
                                  ...
                                  Container(
                                    ...
                                    width: AppConstants.itemWidth*0.40,
                                    // margin: EdgeInsets.only(left: AppConstants.itemWidth*0.01),
                                    child: Text("${invoice.mainInvoiceStockList[index].strItemName}",maxLines:1,overflow: TextOverflow.visible,style: montserratBlack.copyWith(
                                        fontWeight: FontWeight.w600
                                    )),
                                  ),
                                ],
                          ...
                          Column(
                            ...
                            children: [
                              Container(
                                alignment: Alignment.centerRight,
                                width: AppConstants.itemWidth*0.30,
                                child: Text("\u20b9 ${invoice.mainInvoiceStockList[index].decAmount!.round()}",style: montserratBlack.copyWith(
                                    fontWeight: FontWeight.w600
                                )),
                              ),
                              Container(
                                alignment: Alignment.centerRight,
                                width: AppConstants.itemWidth*0.40,
                                child: Text("${invoice.mainInvoiceStockList[index].decQty!.round()} ${invoice.mainInvoiceStockList[index].strUnitName} x ${invoice.mainInvoiceStockList[index].decRate!.round()} = \u20b9 ${invoice.mainInvoiceStockList[index].decAmount!.round()}",
                                    maxLines: 1,
                                    overflow: TextOverflow.ellipsis,
                                    style: montserratRegular.copyWith(
                                    fontWeight: FontWeight.w400
                                    ...
                                    return AlertDialog(
                                      ...
                                      actions: [
                                        TextButton(
                                          onPressed: () {
                                            setState(() {
                                              grandTotal = double.parse(subTotalController.text) - int.parse(invoice.mainInvoiceStockList[index].decAmount!.round().toString());
                                              subTotal = subTotal - grandTotal;
                                              subTotalController.text = grandTotal.round().toString();
                                              grandTotalController.text = grandTotal.round().toString();
                                              print(" :: $items");
                                              items.add(invoice.mainInvoiceStockList[index]);
                                              Provider.of<InvoiceProvider>(context, listen: false).mainInvoiceStockList.removeWhere((element){
                                                return element.intid == invoice.mainInvoiceStockList[index].intid;
                                              });
                                              setState(() {
                                                for(int i=0;i<items.length;i++){
                                                  print(" :::  ${items[i].intid}");
                ...
                // MyDataTableMerge(widget.id,dropdownValue.toString(),customerName.toString(),dateController.text,dueDateController.text,subTotalController.text,discountController.text,grandTotalController.text,grandTotal.toString(),modelList,invoice.mainInvoiceStockList),
...
class MyDataTableMerge extends StatefulWidget {
  String id,customerId,customerName,invoiceDate,dueDate,subTotal,discount,grandTotal,totalGrand;
  List<MainInvoiceStockBody> modelList;
  List<MainInvoiceStockData> stockList;
  MyDataTableMerge(this.id,this.customerId,this.customerName,this.invoiceDate,this.dueDate,this.subTotal,this.discount,this.grandTotal,this.totalGrand,this.modelList,this.stockList,{super.key});
...
class _MyDataTableMergeState extends State<MyDataTableMerge> {
  InvoiceStockDb dbManager = new InvoiceStockDb();
  MainInvoiceStockBody? model;
  List<MainInvoiceStockBody>? driverStockModel;
  List<MainInvoiceStockBody> modelList = new List.empty(growable: true);
  ...
  void initState() {
    // TODO: implement initState
    super.initState();
    dbManager.getInvoiceStockList();
    setState(() {
      dbManager = InvoiceStockDb();
      dbManager.getInvoiceStockList().then((value) {
        setState(() {
          modelList.addAll(value);
          modelList = value;
                        ...
                        TextButton(
                          onPressed: () {
                            setState(() {
                              ...
                              // widget.subTotal = grandTotal.round().toString();
                              // widget.grandTotal = grandTotal.round().toString();
                              grandTotal = double.parse(widget.subTotal) - int.parse(item.decAmount.toString());
                              subTotal = subTotal - grandTotal;
                              ...
                              // widget.subTotal = subTotal.round().toString();
                              // widget.grandTotal = subTotal.round().toString();
                              dbManager.deleteInvoiceStock(item).then((value) {
                                Navigator.push(context, MaterialPageRoute(builder: (context) => EditInvoice(widget.id,widget.customerId,widget.customerName,widget.invoiceDate,widget.dueDate,subTotal.toString(),widget.discount,grandTotal.toString(),"",modelList,"table"),));
                              });
                              Navigator.pop(context);
                            });
                          },
                      ...
                      TextButton(
                        onPressed: () {
                          setState(() {
                            ...
                            widget.subTotal = subTotal.round().toString();
                            widget.grandTotal = subTotal.round().toString();
                            print("stock:::$subTotal::${widget.subTotal}:::$grandTotal:::${widget.grandTotal}:::${widget.totalGrand}:::${item.decAmount!.round().toString()}");
                            // grandTotal = double.parse(widget.subTotal) - int.parse(item.decAmount!.round().toString());
                            // subTotal = subTotal - grandTotal;
                            // widget.subTotal = subTotal.round().toString();
                            // widget.grandTotal = subTotal.round().toString();
