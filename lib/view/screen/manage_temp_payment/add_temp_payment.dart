...
class AddTempPayment extends StatefulWidget {
  int customerId,type;
  AddTempPayment(this.customerId,this.type,{Key? key}) : super(key: key);

  @override
  State<AddTempPayment> createState() => _AddTempPaymentState();
}

class _AddTempPaymentState extends State<AddTempPayment> {
  ...
  void initState() {
    ...
    dropdownValue = widget.type == 1 ? widget.customerId : null;
    dateController.text = formattedDate;
      ...
      Provider.of<RouteProvider>(context, listen: false).getSelectRoute(context,PreferenceUtils.getString("${AppConstants.companyId}")).then((value) {
        setState(() {
          ...
          for(int k=0;k<Provider.of<RouteProvider>(context, listen: false).selectrouteList.length;k++){
            ...
            print("object::::1:::${Provider.of<RouteProvider>(context, listen: false).selectrouteList[k].intId}:::::${Provider.of<RouteProvider>(context, listen: false).selectrouteList[k].strRoute.toString()}");
  ...
  _addPayment(){
    ...
    // }
      ...
      AddPayment_Body payment = AddPayment_Body(
        ...
        intTranscationType: 1,
        strRemarks: "",
      );
      Provider.of<PaymentProvider>(context, listen: false).getAddPayment(context,payment,_route);
  ...
  _route(isRoute){
    if (isRoute) {
      ...
      AppConstants.getToast("Payment Added Successfully");
    }  else{
          ...
          Container(
            ...
            child: Consumer<RouteProvider>(builder: (context, route, child) {
                for(int k=0;k<Provider.of<RouteProvider>(context, listen: false).driverList.length;k++){
                  if (widget.customerId == Provider.of<RouteProvider>(context, listen: false).driverList[k].intId) {
